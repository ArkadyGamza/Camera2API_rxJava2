 ...
 */
@TargetApi(21) class CameraStrategy {
  ...
  @Nullable
  static String switchCamera(@NonNull CameraManager manager, @Nullable String currentCameraId)
      throws CameraAccessException {
    if (currentCameraId != null) {
      Integer currentFacing = manager.getCameraCharacteristics(currentCameraId)
  ...
  @Nullable
  private static String getCameraWithFacing(@NonNull CameraManager manager, int lensFacing)
      throws CameraAccessException {
  ...
  static Size getPreviewSize(@NonNull CameraCharacteristics characteristics) {
    ...
    List<Size> filteredOutputSizes = Observable.fromArray(outputSizes)
        .filter(
            size -> size.getWidth() <= MAX_PREVIEW_WIDTH && size.getHeight() <= MAX_PREVIEW_HEIGHT)
        .toList()
        .blockingGet();
  ...
  /**
   * Please note that aspect ratios should be the same for {@link #getPreviewSize(CameraCharacteristics)}
   * and {@link #getStillImageSize(CameraCharacteristics, Size)}
   */
  static Size getStillImageSize(@NonNull CameraCharacteristics characteristics,
    ...
    List<Size> filteredOutputSizes = Observable.fromArray(outputSizes)
        .filter(size -> size.getWidth() == size.getHeight() * previewSize.getWidth() / previewSize
            .getHeight())
        .filter(size -> size.getWidth() <= MAX_STILL_IMAGE_WIDTH
            && size.getHeight() <= MAX_STILL_IMAGE_HEIGHT)
        .toList()
        .blockingGet();
