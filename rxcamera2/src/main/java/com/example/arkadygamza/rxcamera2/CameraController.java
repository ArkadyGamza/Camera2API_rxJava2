...
public class CameraController {
  ...
  private class CameraParams {
    ...
    private CameraParams(@NonNull String cameraId,
        @NonNull CameraCharacteristics cameraCharacteristics, @NonNull Size previewSize) {
  ...
  private final ConvergeWaiter mAutoFocusConvergeWaiter = ConvergeWaiter.Factory
      .createAutoFocusConvergeWaiter();
  private final ConvergeWaiter mAutoExposureConvergeWaiter = ConvergeWaiter.Factory
      .createAutoExposureConvergeWaiter();


  public CameraController(@NonNull Context context, @NonNull Callback callback,
      @NonNull String photoFileUrl,
  ...
  private final LifecycleObserver mLifecycleObserver = new DefaultLifecycleObserver() {
    ...
    public void onCreate(@NonNull LifecycleOwner owner) {
      ...
      mTextureView.addOnLayoutChangeListener(
  ...
  private void setTextureAspectRatio(@NonNull CameraParams cameraParams) {
    ...
    if (mLayoutOrientation == Configuration.ORIENTATION_LANDSCAPE) {
      mTextureView.setAspectRatio(cameraParams.previewSize.getWidth(),
          cameraParams.previewSize.getHeight());
    } else {
      mTextureView.setAspectRatio(cameraParams.previewSize.getHeight(),
          cameraParams.previewSize.getWidth());
    }
  }
  ...
  private void subscribe() {
    ...
    Observable<CaptureSessionData> previewObservable = captureSessionConfiguredObservable
        .flatMap(cameraCaptureSession -> {
          ...
          CaptureRequest.Builder previewBuilder = createPreviewBuilder(cameraCaptureSession,
              mSurface);
    ...
    mCompositeDisposable.add(
        Observable.combineLatest(previewObservable, mOnShutterClick,
            (captureSessionData, o) -> captureSessionData)
    ...
    mCompositeDisposable.add(
        Observable.combineLatest(previewObservable, mOnSwitchCameraClick,
            (captureSessionData, o) -> captureSessionData)
            .firstElement().toObservable()
    ...
    mCompositeDisposable
        .add(Observable.combineLatest(previewObservable, mOnPauseSubject, (state, o) -> state)
            .firstElement().toObservable()
  ...
  private void setupSurface(@NonNull SurfaceTexture surfaceTexture) {
    surfaceTexture.setDefaultBufferSize(mCameraParams.previewSize.getWidth(),
  ...
  private void initImageReader() {
    ...
    Size sizeForImageReader = CameraStrategy
        .getStillImageSize(mCameraParams.cameraCharacteristics, mCameraParams.previewSize);
    ...
    mCompositeDisposable.add(
        ImageSaverRxWrapper.createOnImageAvailableObservable(mImageReader)
            ...
            .flatMap(
                imageReader -> ImageSaverRxWrapper.save(imageReader.acquireLatestImage(), mFile)
                    .toObservable())
            .observeOn(AndroidSchedulers.mainThread())
            .subscribe(
                file -> mCallback.onPhotoTaken(file.getAbsolutePath(), getLensFacingPhotoType()))
    );
  }
  ...
  private Observable<CaptureSessionData> waitForAf(
      @NonNull CaptureSessionData captureResultParams) {
    return Observable
  ...
  @NonNull
  private Observable<CaptureSessionData> waitForAe(
      @NonNull CaptureSessionData captureResultParams) {
    return Observable
        .fromCallable(() -> createPreviewBuilder(captureResultParams.session, mSurface))
        .flatMap(
  ...
  @NonNull
  private Observable<CaptureSessionData> captureStillPicture(
  ...
  @NonNull
  private CaptureRequest.Builder createStillPictureBuilder(@NonNull CameraDevice cameraDevice)
    ...
    builder.set(CaptureRequest.JPEG_ORIENTATION,
        CameraOrientationHelper.getJpegOrientation(mCameraParams.cameraCharacteristics, rotation));
    return builder;
  }

  @NonNull
  CaptureRequest.Builder createPreviewBuilder(CameraCaptureSession captureSession,
      Surface previewSurface) throws CameraAccessException {
    CaptureRequest.Builder builder = captureSession.getDevice()
  ...
  private void setup3Auto(CaptureRequest.Builder builder) {
    ...
    Float minFocusDist = mCameraParams.cameraCharacteristics
    ...
    if (!noAFRun) {
      ...
      int[] afModes = mCameraParams.cameraCharacteristics
    ...
    int[] aeModes = mCameraParams.cameraCharacteristics
    ...
    int[] awbModes = mCameraParams.cameraCharacteristics
