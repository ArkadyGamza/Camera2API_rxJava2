...
public class CameraRxWrapper {
    ...
    return Observable.create(observableEmitter -> {
      ...
      cameraManager.openCamera(cameraId, new CameraDevice.StateCallback() {
        ...
        public void onError(@NonNull CameraDevice camera, int error) {
          ...
          if (!observableEmitter.isDisposed()) {
            observableEmitter
                .onError(new OpenCameraException(OpenCameraException.Reason.getReason(error)));
          }
        }
      }, null);
    });
  ...
  public enum CaptureSessionStateEvents {
    ...
    ON_CLOSED,
    /**
     * @see CameraCaptureSession.StateCallback#onSurfacePrepared(CameraCaptureSession,
    ...
    return Observable.create(observableEmitter -> {
      ...
      cameraDevice.createCaptureSession(surfaceList, new CameraCaptureSession.StateCallback() {
        ...
        public void onSurfacePrepared(@NonNull CameraCaptureSession session,
          ...
          if (!observableEmitter.isDisposed()) {
            observableEmitter
                .onNext(new Pair<>(CaptureSessionStateEvents.ON_SURFACE_PREPARED, session));
          }
        }
      }, null);
    });
  ...
  public static class CaptureSessionData {
    ...
    CaptureSessionData(CaptureSessionEvents event, CameraCaptureSession session,
   ...
   * Warning, emits a lot!
   */
  static Observable<CaptureSessionData> fromSetRepeatingRequest(
      @NonNull CameraCaptureSession captureSession, @NonNull CaptureRequest request) {
    return Observable
        .create(observableEmitter -> captureSession
            .setRepeatingRequest(request, createCaptureCallback(observableEmitter), null));
  }

  static Observable<CaptureSessionData> fromCapture(@NonNull CameraCaptureSession captureSession,
      @NonNull CaptureRequest request) {
    return Observable
        .create(observableEmitter -> captureSession
            .capture(request, createCaptureCallback(observableEmitter), null));
  }

  @NonNull
  private static CameraCaptureSession.CaptureCallback createCaptureCallback(
      final ObservableEmitter<CaptureSessionData> observableEmitter) {
      ...
      public void onCaptureCompleted(@NonNull CameraCaptureSession session,
        ...
        if (!observableEmitter.isDisposed()) {
          observableEmitter.onNext(
              new CaptureSessionData(CaptureSessionEvents.ON_COMPLETED, session, request, result));
        }
      }
